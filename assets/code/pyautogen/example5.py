...
llm_config_mixtral={
    "config_list": config_list_mixtral,
}
...
- find all the files installed via apt or dpkg and avoid to use sudo. 
- if any file is not accessible just log it name, path, size and modication time to anylize it later
- check the md5sum of the installed files and compare it with what reported by the dpkg database.
- identify the package and files that are different from the initial installation
...
- The output shoult be printed in the console and also written in an output file. Aggretate the data to make it better understandable
